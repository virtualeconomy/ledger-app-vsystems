/*******************************************************************************
*   vsys platform Wallet App for Nano Ledger S. Updated By vsys community.
...
void add_chunk_data() {
    ...
    if (tmp_ctx.signing_context.buffer_used == 0) {
        ...
        // 21 byte - data type
        tmp_ctx.signing_context.data_type = G_io_apdu_buffer[25];
        if (!is_tx_type_supported(tmp_ctx.signing_context.data_type)) {
            THROW(SW_FUNC_NOT_SUPPORTED);
        }
        // 22 byte - data version
        tmp_ctx.signing_context.data_version = G_io_apdu_buffer[26];
        if (tmp_ctx.signing_context.data_version > SUPPORTED_DATA_VERSION) {
            THROW(SW_VERSION_TOO_LOW);
        }
...
void parse_tx_context(signingContext_t *context) {
    context->amount = get_amount_from_buff(context->buffer, context->data_type);
    get_recipient_from_buff(context->buffer, context->data_type, context->to);
    get_tx_id_from_buff(context->buffer, context->data_type, context->id);
}
...
uint32_t set_result_sign() {
    ...
    vsys_message_sign(&private_key, public_key.W, (unsigned char *) tmp_ctx.signing_context.buffer, tmp_ctx.signing_context.buffer_used, signature);
        ...
        TRY {
            ...
            case INS_SIGN: {
                ...
                if (G_io_apdu_buffer[2] == P1_LAST) {
                    ...
                    add_chunk_data();
                    parse_tx_context(&tmp_ctx.signing_context);
                    menu_sign_init();
            ...
            case INS_GET_PUBLIC_KEY: {
                ...
                vsys_public_key_to_address(public_key.W, G_io_apdu_buffer[3], address);
